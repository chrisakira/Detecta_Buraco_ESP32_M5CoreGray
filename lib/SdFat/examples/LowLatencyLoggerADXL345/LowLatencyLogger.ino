...
void checkOverrun() {
  ...
  while (binFile_.read(&block, 512) == 512) {
    if (block.count == 0) {
      break;
    }
    if (block.overrun) {
      ...
      Serial.print(bn);
      Serial.print(',');
      Serial.print(binFile_.firstBlock() + bn);
      Serial.print(',');
...
void binaryToCsv() {
  ...
  // Create a new csvFile_.
  strcpy(csvName, binName);
  strcpy(&csvName[BASE_NAME_SIZE + 3], "csv");

  if (!csvFile_.open(csvName, O_WRONLY | O_CREAT | O_TRUNC)) {
    error("open csvFile_ failed");
  }
  binFile_.rewind();
  Serial.print(F("Writing: "));
  Serial.print(csvName);
  Serial.println(F(" - type any character to stop"));
  printHeader(&csvFile_);
  uint32_t tPct = millis();
  while (!Serial.available() && binFile_.read(&block, 512) == 512) {
    uint16_t i;
    if (block.count == 0 || block.count > DATA_DIM) {
      break;
    }
    if (block.overrun) {
      csvFile_.print(F("OVERRUN,"));
      csvFile_.println(block.overrun);
    }
    for (i = 0; i < block.count; i++) {
      printData(&csvFile_, &block.data[i]);
    }
    if (csvFile_.curCluster() != syncCluster) {
      csvFile_.sync();
      syncCluster = csvFile_.curCluster();
    }
    if ((millis() - tPct) > 1000) {
      uint8_t pct = binFile_.curPosition()/(binFile_.fileSize()/100);
      if (pct != lastPct) {
        tPct = millis();
        lastPct = pct;
...
void createBinFile_() {
  ...
  Serial.println(F("\nCreating new file"));
  binFile_.close();
  if (!binFile_.createContiguous(TMP_FILE_NAME, 512 * FILE_BLOCK_COUNT)) {
    error("createContiguous failed");
  }
  // Get the address of the file on the SD.
  if (!binFile_.contiguousRange(&bgnBlock, &endBlock)) {
...
void openBinFile_() {
  ...
  if (!sd.exists(name)) {
    Serial.println(F("File_ does not exist"));
    return;
  }
  binFile_.close();
  strcpy(binName, name);
  if (!binFile_.open(binName, O_RDONLY)) {
    Serial.println(F("open failed"));
    return;
  }
  Serial.println(F("File_ opened"));
}
//------------------------------------------------------------------------------
void recordBinFile_() {
  ...
  if (!sd.card()->writeStart(binFile_.firstBlock())) {
    error("writeStart failed");
  }
  Serial.print(F("FreeStack: "));
  Serial.println(FreeStack());
  Serial.println(F("Logging - type any character to stop"));
  bool closeFile_ = false;
  uint32_t bn = 0;
  ...
  if (bn != FILE_BLOCK_COUNT) {
    ...
    if (!binFile_.truncate(512L * bn)) {
      error("Can't truncate file");
    }
  }
}
//------------------------------------------------------------------------------
void recoverTmpFile_() {
  uint16_t count;
  if (!binFile_.open(TMP_FILE_NAME, O_RDWR)) {
    return;
  }
  if (binFile_.read(&count, 2) != 2 || count != DATA_DIM) {
    error("Please delete existing " TMP_FILE_NAME);
  }
  Serial.println(F("\nRecovering data in tmp file " TMP_FILE_NAME));
  uint32_t bgnBlock = 0;
  uint32_t endBlock = binFile_.fileSize()/512 - 1;
  // find last used block.
  while (bgnBlock < endBlock) {
    uint32_t midBlock = (bgnBlock + endBlock + 1)/2;
    binFile_.seekSet(512*midBlock);
    if (binFile_.read(&count, 2) != 2) error("read");
  ...
  if (!binFile_.truncate(512*(bgnBlock + 1))) {
...
void renameBinFile_() {
  ...
  if (!binFile_.rename(binName)) {
    error("Can't rename file");
    }
  Serial.print(F("File_ renamed: "));
  Serial.println(binName);
  Serial.print(F("File_ size: "));
  Serial.print(binFile_.fileSize()/512);
