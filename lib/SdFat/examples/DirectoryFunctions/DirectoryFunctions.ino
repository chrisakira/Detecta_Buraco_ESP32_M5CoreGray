...
void setup() {
  ...
  if (sd.exists("Folder1")
    ...
    || sd.exists("Folder1/File_2.txt")) {
    error("Please remove existing Folder1, file1.txt, and File_2.txt");
  }

  int rootFile_Count = 0;
  if (!root.open("/")) {
  ...
  // Create File_2.txt in current directory.
  if (!file.open("File_2.txt", O_WRONLY | O_CREAT)) {
    error("create File_2.txt failed");
  }
  file.close();
  cout << F("Created File_2.txt in current directory\n");

  cout << F("\nList of files on the SD.\n");
  sd.ls("/", LS_R);

  // Remove files from current directory.
  if (!sd.remove("file1.txt") || !sd.remove("File_2.txt")) {
    error("remove failed");
  }
  cout << F("\nfile1.txt and File_2.txt removed.\n");
